...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    while True:
        ...
        for s in range(nS):
            Vs = 0
            for a in range(nA):
                pi = policy[s][a] #policy pi(a|s)
                for prob, next_state, reward, terminal in P[s][a]:
                    Vs += pi * prob * (reward + gamma * value_function[next_state]) #equation for value function
...
def policy_improvement(P, nS, nA, value_from_policy, gamma=0.9):
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            reward =0
            for prob, next_state, reward, terminal in P[s][a]:
                Vs[a] += prob*(reward + gamma * value_from_policy[next_state]) #equation for value function
        new_policy[s] = np.eye(nA)[np.argmax(Vs)] #update policy
...
def policy_iteration(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    while True:
        policy = new_policy
        V = policy_evaluation(P, nS, nA, policy, gamma, tol) #evaluate value function
        new_policy = policy_improvement(P, nS, nA, V, gamma) #improve policy
    ...
    while(True):
        ...
        for s in range(nS):
            ...
            for a in range(nA):
                value = 0
                for prob, next_state, reward, terminal in P[s][a]: 
                    value += prob*(reward + gamma * V_new[next_state]) 
                action_value.append(value)
            max_a_idx = np.argmax(action_value) #find the best action
            policy_new[s][max_a_idx] = 1 #update policy
            V_new[s] = action_value[max_a_idx] #update value function
            delta=max(delta,abs(v1-V_new[s])) 
        if(delta<tol):
            break
